...
async function fetchWithRetry(url: string, headers: Record<string, string>, body: string, retries = 2): Promise<Response> {
...
serve(async (req) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(`[resume-chat] AI Gateway error: ${response.status}`, errBody.slice(0, 300));
